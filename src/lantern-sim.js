// An in-memory Lantern ledger that runs the real generated circuits. Every
// call is made AS a persona: that persona's private state is what the
// witnesses see, and nothing else.
//
// Runtime-free: the runtime and the contract module are passed in, so the
// browser, the tests and the terminal story can share it.

const COIN_PK = '0'.repeat(64);

export function createLanternSim({ rt, mod, witnesses, now = 1_700_000_000 }) {
  const contract = new mod.Contract(witnesses);
  const ctor = contract.initialState(rt.createConstructorContext({}, COIN_PK));
  let clock = now;
